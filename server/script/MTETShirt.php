<?php
	...
	class MTETShirt
	{
		static function clean_tshirt_from_db($tshirt)
		{
			...
			$tshirt = MTEToolbox::rename_keys($tshirt, array('tsh_id', 'tsh_name', 'tsh_size', 'tsh_color', 'tsh_condition', 'tsh_rating', 'tsh_tags', 'tsh_store_id', 'tsh_note', 'tsh_img_close'), array('identifier', 'name', 'size', 'color', 'condition', 'rating', 'tags', 'store_id', 'note', 'image_url'), true);
		...
		static function clean_wear_from_db($wear)
		{
			$wear = MTEToolbox::rename_keys($wear, array('wea_id', 'wea_date'), array('identifier', 'date'), true);
			
			return $wear;
		}
		
		static function clean_wash_from_db($wash)
		{
			$wash = MTEToolbox::rename_keys($wash, array('was_id', 'was_date'), array('identifier', 'date'), true);
			
			return $wash;
		}
		
		static function fectch_wash_wear_store_for_tshirt($database, $tshirt)
		{
			...
			if (count($stores)==1)
			{
				$tshirt['store'] = MTEStore::clean_store_from_db($stores[0]);
			}
